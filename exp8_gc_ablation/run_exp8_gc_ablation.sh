...
printf "workload\tbackup_method\tgc_method\tablation_config\tthroughput_kops\tops_file\n" > "${throughput_file}"
printf "workload\tbackup_method\tgc_method\tablation_config\tstatus\ttotal_used_bytes\ttotal_used_gib\tper_host_used_bytes\tper_host_used_gib\ttotal_disk_write_bytes\ttotal_disk_write_gib\tper_host_disk_write_bytes\tper_host_disk_write_gib\tper_host_disk_write_pids\tserver_log_path\n" > "${summary_file}"
printf "workload\tbackup_method\tgc_method\tablation_config\thost\tstatus\tlast_timestamp\tused_bytes\tused_gib\tprev_used_bytes\tprev_used_gib\tdisk_write_pids\tdisk_write_bytes\tdisk_write_gib\tserver_log_path\n" > "${host_file}"
...
cleanup() {
    ...
    if [[ ${servers_may_be_running} -eq 1 ]]; then
        echo "Stopping tebis servers from exp8 cleanup..." >&2
        stop_servers
        servers_may_be_running=0
    fi
...
set_fixed_exp_params() {
    echo "Setting VALID_SEGMENTS_THRESHOLD=${valid_segments_threshold_value}, high_amplification_power=${high_amplification_power_value} before exp8_gc_ablation build..." >&2
...
restore_default_build() {
    ...
    echo "Rebuilding with default scp.sh flags after exp8_gc_ablation..." >&2
    cmake_args=()
    sync_and_build_all_nodes
    high_amplification_build_enabled=0
}

set_ablation_cmake_args() {
    local ablation_config=$1
    ...
    both)
        ...
        ;;
    *)
        echo "Unsupported ablation config: ${ablation_config}" >&2
        exit 1
        ;;
    esac
}
...
write_host_rows() {
    local workload=$1
    local ablation_config=$2
    local remote_log=$3
    local host

    for host in "${hosts[@]}"; do
        printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
            "${workload}" "${backup_method}" "${gc_method}" "${ablation_config}" "${host}" \
            "${LAST_HOST_STATUS[${host}]}" \
...
if [[ ${#ablation_configs[@]} -ne ${#ablation_labels[@]} ]]; then
    echo "ablation_configs and ablation_labels must have the same length." >&2
    exit 1
fi
...
echo "Running exp8 gc ablation (backup=${backup_method}), gc_method=${gc_method}, ablation_configs=${ablation_configs[*]}, valid_segments_threshold=${valid_segments_threshold_value}, high_amplification_power=${high_amplification_power_value}, workloads=${workloads[*]}"

for workload in "${workloads[@]}"; do
    for ablation_config in "${ablation_configs[@]}"; do
        echo -e "\n*********Running exp8_gc_ablation workload=${workload}, backup=${backup_method}, gc=${gc_method}, ablation=${ablation_config}*********"

        set_ablation_cmake_args "${ablation_config}"
        set_fixed_exp_params
        sync_and_build_all_nodes
        high_amplification_build_enabled=1

        run_tag="${date_time}_${gc_method}_${ablation_config}"
        output_base="${results_rel}/client_logs/${nickname}_${backup_method}_${gc_method}_${ablation_config}_${workload}_thread_${server_threads}_${client_threads}"
        output_path="${project_dir}/${output_base}_${run_tag}"
        server_log_path="/tmp/lijinming_tebis_server_exp8_${backup_method}_${gc_method}_${ablation_config}_${workload}_${run_tag}.log"

        echo "Running workload=${workload}, backup=${backup_method}, gc=${gc_method}, ablation=${ablation_config}" >&2
        ...
        printf "%s\t%s\t%s\t%s\t%s\t%s\n" "${workload}" "${backup_method}" "${gc_method}" "${ablation_config}" "${kops}" "${ops_file}" >> "${throughput_file}"
        ...
        printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
            "${workload}" "${backup_method}" "${gc_method}" "${ablation_config}" "${SPACE_STATUS}" \
            ...
            >> "${summary_file}"
        echo "Space usage result: workload=${workload}, backup=${backup_method}, gc=${gc_method}, ablation=${ablation_config}, total_used_gib=${SPACE_TOTAL_USED_GIB}, total_disk_write_gib=${DISK_TOTAL_WRITE_GIB}" >&2

        write_host_rows "${workload}" "${ablation_config}" "${server_log_path}"
...
ablation_config_csv=$(join_csv "${ablation_configs[@]}")
bar_label_csv=$(join_csv "${display_workload_labels[@]}")
ablation_label_csv=$(join_csv "${ablation_labels[@]}")

echo ""
echo "Executing command:"
echo "python3 \"${basic_script_dir}/plot_exp2_space_bar.py\" --input \"${summary_file}\" --output \"${results_dir}\" --workloads \"${workload_csv}\" --backups \"${ablation_config_csv}\" --item-column ablation_config --bar-label \"${bar_label_csv}\" --item-labels \"${ablation_label_csv}\" --x-axis-label \"Workload\" --y-axis-label \"Space usage (GiB)\" --value-column total_used_gib --output-name space_occupation.pdf --legend-output-name space_occupation_legend.pdf"

python3 "${basic_script_dir}/plot_exp2_space_bar.py" \
    --input "${summary_file}" \
    --output "${results_dir}" \
    --workloads "${workload_csv}" \
    --backups "${ablation_config_csv}" \
    --item-column ablation_config \
    --bar-label "${bar_label_csv}" \
    --item-labels "${ablation_label_csv}" \
...
echo ""
echo "Executing command:"
echo "python3 \"${basic_script_dir}/plot_exp2_space_bar.py\" --input \"${summary_file}\" --output \"${results_dir}\" --workloads \"${workload_csv}\" --backups \"${ablation_config_csv}\" --item-column ablation_config --bar-label \"${bar_label_csv}\" --item-labels \"${ablation_label_csv}\" --x-axis-label \"Workload\" --y-axis-label \"Disk writes (GiB)\" --value-column total_disk_write_gib --output-name disk_write.pdf --legend-output-name disk_write_legend.pdf"
...
echo "exp8 gc ablation finished. Results: ${results_dir}"
